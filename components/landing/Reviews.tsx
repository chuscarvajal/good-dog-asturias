import { Star } from "lucide-react";

const reviews = [
  {
    name: "Carlos M.",
    initials: "CM",
    text: "Increíble experiencia. Nuestro perro tenía problemas de agresividad con otros perros y en pocas sesiones vimos un cambio enorme. El trato es cercano, profesional y siempre con mucha paciencia.",
    rating: 5,
  },
  {
    name: "Laura P.",
    initials: "LP",
    text: "Lo mejor que hemos hecho por nuestro perro. Explican todo muy bien, te dan herramientas para trabajar en casa y los resultados se notan desde el primer día. 100% recomendable.",
    rating: 5,
  },
  {
    name: "Javier R.",
    initials: "JR",
    text: "Muy contentos con el trabajo realizado. Nuestro cachorro ahora obedece órdenes básicas y ha mejorado muchísimo en la socialización con otros perros. Gran profesional.",
    rating: 5,
  },
];

function Stars({ count }: { count: number }) {
  return (
    <div className="flex gap-0.5">
      {Array.from({ length: count }).map((_, i) => (
        <Star key={i} size={14} className="fill-[#f5a623] text-[#f5a623]" />
      ))}
    </div>
  );
}

export default function Reviews() {
  return (
    <section className="py-16 lg:py-24 bg-[#2b2b2b]">
      <div className="container">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-12">
          <div>
            <h2 className="text-white font-black text-2xl sm:text-3xl uppercase tracking-[0.1em]">
              Reseñas de Google
            </h2>
            <p className="text-white/50 text-sm mt-2">Lo que dicen nuestros clientes</p>
          </div>
          <a
            href="https://www.google.com/maps/search/Good+Dog+Allende+Asturias"
            target="_blank"
            rel="noopener noreferrer"
            className="self-start md:self-auto flex items-center gap-2 text-white/60 text-xs uppercase tracking-widest border border-white/20 px-5 py-2.5 hover:bg-white hover:text-[#2b2b2b] transition-colors"
          >
            Ver todas en Google
          </a>
        </div>

        <div className="grid md:grid-cols-3 gap-6">
          {reviews.map((r) => (
            <div key={r.name} className="bg-white/5 border border-white/10 p-6 flex flex-col gap-4">
              <Stars count={r.rating} />
              <p className="text-white/70 text-sm leading-relaxed flex-1">&ldquo;{r.text}&rdquo;</p>
              <div className="flex items-center gap-3 pt-2 border-t border-white/10">
                <div className="w-9 h-9 rounded-full bg-white/15 flex items-center justify-center text-white text-xs font-bold shrink-0">
                  {r.initials}
                </div>
                <div>
                  <p className="text-white text-xs font-semibold">{r.name}</p>
                  <p className="text-white/40 text-[10px] uppercase tracking-widest">Google</p>
                </div>
              </div>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
}
